...
const HIGH_INTENT_KEYWORDS = [
  "จอง", "มัดจำ", "โอนเงิน", "จัดไฟแนนซ์", "ส่งเอกสาร",
  "วางมัดจำ", "โอนมัดจำ", "พร้อมจอง", "จ่ายเงินดาวน์", "วางเงินดาวน์",
  "ยืนยันซื้อ", "ขอจอง", "จองคิว", "สลิปโอน", "ปิดการขาย", "จ่ายจอง",
  "โอนแล้ว", "จ่ายมัดจำ", "พร้อมซื้อ", "ตกลงซื้อ", "เอาคันนี้เลย",
];
...
const SERVICE_KEYWORDS_RE = /ซ่อม|เช็คระยะ|อะไหล่|คิวซ่อม|นัดซ่อม|เข้าศูนย์|เปลี่ยนน้ำมันเครื่อง|เช็คสภาพ|ตรวจเช็ค|ยางรั่ว|เบรค|เบรก|โซ่|แบตเตอรี่|แบตหมด|สตาร์ทไม่ติด|เสียงดังผิดปกติ|ไฟไม่ติด|ประกันศูนย์|พ่วงแบต|รถเสีย|ซ่อมด่วน|เข้าเช็ค|นัดคิวช่าง/;
const TRADE_IN_KEYWORDS_RE = /เทิร์นรถ|เทิร์น|แลกรถ|ขายรถเก่า|ประเมินราคารถ|รับซื้อรถ|เอารถเก่ามาแลก|ตีเทิร์น|ตีราคารถเก่า|ขายคืนรถ/;
const BUYING_NEW_KEYWORDS_RE = /ซื้อรถ|ออกรถ|จองรถ|ดาวน์รถ|สนใจรุ่น|คันใหม่|รถใหม่|ผ่อนรถ|ราคารถ|ราคาผ่อน|งวดผ่อน|เงินดาวน์|ดอกเบี้ย|ไฟแนนซ์รถใหม่|สเปครถ|ทดลองขับ|เทสไดร์ฟ|มีสีอะไรบ้าง|ตัวท็อป|รุ่นท็อป|สั่งจอง|จองคิวรับรถ|ราคาเงินสด|อยากได้คันใหม่|รถป้ายแดง/;

function guessIntentFromText(text) {
  if (!text) return null;
  if (SERVICE_KEYWORDS_RE.test(text)) return "service";
  if (TRADE_IN_KEYWORDS_RE.test(text)) return "trade_in";
  if (BUYING_NEW_KEYWORDS_RE.test(text)) return "buying_new";
  return null;
}

function intentKeywordMatches(intent, text) {
  if (!text) return false;
  if (intent === "service") return SERVICE_KEYWORDS_RE.test(text);
  if (intent === "trade_in") return TRADE_IN_KEYWORDS_RE.test(text);
  if (intent === "buying_new") return BUYING_NEW_KEYWORDS_RE.test(text);
  return true;
}
